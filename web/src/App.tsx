...
export function App() {
  ...
  const onUpdateUser = useCallback((user: UserInfo) => {
    setSession((prev) => {
      if (!prev) return prev;
  ...
  return session ? (
    <Dashboard session={session} onLogout={onLogout} onUpdateUser={onUpdateUser} />
  ) : (
    ...
    <div className="app">
      ...
      {showSettings && (
        <SettingsModal
          ...
          user={session.user}
          onUpdateUser={(u) => {
            const filterChanged = u.filterSensitive !== session.user.filterSensitive;
            onUpdateUser(u);
            if (filterChanged) {
              const q = query.trim();
              if (q) void runSearch(q);
...
function errMsg(e: unknown): string {
  return e instanceof WtmApiError ? e.message : "Request failed.";
}
...
function SettingsModal({
  ...
  client: ReturnType<typeof clientFor>;
  user: UserInfo;
  onUpdateUser: (u: UserInfo) => void;
  onClose: () => void;
}) {
  const [filter, setFilter] = useState(user.filterSensitive);
  const [days, setDays] = useState(String(user.retentionDays));
  ...
  async function toggleFilter() {
    ...
    setError("");
    try {
      onUpdateUser(await client.updateSettings({ filterSensitive: next }));
      flash("Saved");
    } catch (e) {
      setFilter(!next);
      setError(errMsg(e));
  ...
  async function saveRetention() {
    ...
    setError("");
    try {
      onUpdateUser(await client.updateSettings({ retentionDays: d }));
      flash("Saved");
    } catch (e) {
      setError(errMsg(e));
    } finally {
  ...
  async function rename(id: string, name: string) {
    ...
    try {
      ...
      setNodes((prev) => prev?.map((x) => (x.id === id ? updated : x)) ?? prev);
      flash("Renamed");
    } catch (e) {
      setError(errMsg(e));
    }
      ...
      <div className="modal settings" onClick={(e) => e.stopPropagation()}>
        ...
        <section className="setting">
          <label className="toggle">
            <input type="checkbox" checked={filter} disabled={busy} onChange={toggleFilter} />
            <span>
              <b>Hide sensitive pages</b>
              <small>Keep adult / explicit pages out of your timeline and search.</small>
            </span>
          </label>
        </section>
        ...
        <section className="setting">
          ...
          <div className="row">
            ...
            <button className="btn" disabled={busy} onClick={saveRetention}>
              Save
            </button>
          </div>
        </section>
        ...
        <section className="setting">
          ...
          <ul className="devices">
            {nodes == null ? (
              ...
              <li className="muted">No devices yet — sign in from the extension to register one.</li>
            ) : (
              nodes.map((n) => (
                <li key={n.id}>
                  <DeviceRow node={n} onRename={(name) => rename(n.id, name)} />
                </li>
              ))
            )}
          </ul>
...
function DeviceRow({ node, onRename }: { node: NodeInfo; onRename: (name: string) => void }) {
  ...
  return (
    <div className="device-row">
      ...
      <button className="btn" disabled={!dirty} onClick={() => onRename(name)}>
        Rename
      </button>
    </div>
  );
